...
const PARTS = ['Sender', 'Subject', 'Date', 'Recipient', 'Body'];
...
function extractDates(emails) {
  ...
  emails.forEach((email) => {
    const parts = email.split(PART_DELIMITER);
    const dateParts = parts[2].match(/\d{2}-\d{2}-\d{4}/)[0].split('-');
...
function mailCount(emailData) {
  const emails = emailData.split(MESSAGE_DELIMITER);
  const dates = extractDates(emails);
